...
if [ "$cluster_size" == "small" ]; then
	DATANODES=3
	MASTERNODES=3
	BLUEPRINT_FILENAME=./ambari_blueprints/AMBARI_BLUEPRINT_HDP251Hawq21_SMALL.JSON
	BLUEPRINT_TEMPLATE=./ambari_blueprints/AMBARI_BLUEPRINT_HDP251Hawq21_SMALL_TEMPLATE.JSON
	BLUEPRINT_NAME=datalake_blueprint
elif [ "$cluster_size" == "medium" ]; then
	MASTERNODES=4
	DATANODES=6
	BLUEPRINT_FILENAME=./ambari_blueprints/AMBARI_BLUEPRINT_HDP251Hawq21_MEDIUM.JSON
	BLUEPRINT_TEMPLATE=./ambari_blueprints/AMBARI_BLUEPRINT_HDP251Hawq21_MEDIUM_TEMPLATE.JSON
	BLUEPRINT_NAME=hawqdatalake_blueprint
elif [ "$cluster_size" == "large" ]; then
	MASTERNODES=5
	DATANODES=12
	BLUEPRINT_FILENAME=./ambari_blueprints/AMBARI_BLUEPRINT_HDP251Hawq21_LARGE.JSON
	BLUEPRINT_TEMPLATE=./ambari_blueprints/AMBARI_BLUEPRINT_HDP251Hawq21_LARGE_TEMPLATE.JSON
	BLUEPRINT_NAME=hawqdatalake_blueprint
fi
echo "\n"
